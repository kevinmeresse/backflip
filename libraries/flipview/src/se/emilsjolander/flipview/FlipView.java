	...
	private void setFlipDistance(float flipDistance) {
		...
		if (mCurrentPageIndex != currentPageIndex) {
			...
			if (mCurrentPageIndex > 0) {
				...
				Log.i("FlipView", "+++ Adding page " + mPreviousPage.position);
			...
			if (mCurrentPageIndex >= 0 && mCurrentPageIndex < mPageCount) {
				...
				Log.i("FlipView", "+++ Adding page " + mCurrentPage.position);
				addView(mCurrentPage.v);
			}
			if (mCurrentPageIndex < mPageCount - 1) {
				fillPageForIndex(mNextPage, mCurrentPageIndex + 1);
				Log.i("FlipView", "+++ Adding page " + mNextPage.position);
				addView(mNextPage.v);
			}*/
			
			// --------------- DONE -----------------
			// When flipping next
			if (mCurrentPage.valid && mCurrentPageIndex == (mCurrentPage.position + 1)) {
				
				// Swap pointers
				Page temp = mPreviousPage;
				mPreviousPage = mCurrentPage;
				mCurrentPage = mNextPage;
				mNextPage = temp;
				
				// Remove view of old previous page
				if (mNextPage.valid) {
				...
				// Add view of new next page
				if (mCurrentPageIndex < mPageCount - 1) {
					fillPageForIndex(mNextPage, mCurrentPageIndex + 1);
					addView(mNextPage.v);
				}
				
			// When flipping previous
			} else if (mCurrentPage.valid && mCurrentPageIndex == (mCurrentPage.position - 1)) {
				...
				// Add view of new previous page
				if (mCurrentPageIndex > 0) {
					fillPageForIndex(mPreviousPage, mCurrentPageIndex - 1);
					addView(mPreviousPage.v);
				}
				
			// When flipview is empty (or current page is not valid)
			} else {
